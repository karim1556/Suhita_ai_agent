...
export const PlaygroundDeviceSelector = ({
  ...
  return (
    <div className="relative">
      <button
        className="flex gap-2 items-center px-3 py-1.5 bg-gray-900 text-gray-300 border border-gray-800 rounded-md hover:bg-gray-800"
        onClick={(e) => {
      ...
      <div
        className="absolute right-0 mt-2 bg-gray-800 text-gray-300 border border-gray-800 rounded-md z-10 shadow-lg"
        style={{
      ...
      >
        {deviceSelect.devices.map((device, index) => (
          <div
            ...
            className={`text-xs py-2 px-3 cursor-pointer hover:bg-gray-700 hover:text-white ${
              device.deviceId === deviceSelect.activeDeviceId ? "text-white bg-gray-700" : "text-gray-500"
            }`}
            key={index}
          >
            {device.label}
